...
# Updating Vardot 8.1.1 to Vardot 8.1.2


## First way is by updating files with Copy and Paste for Vardot 8.1.1 over Vardoc 8.1.2
```
...
```
  Given that we are at the root folder for Vardoc project, not the docroot
   ...
   When we delete the "vendor" folder
    ...
    And we run the "composer require vardot/vardoc:8.1.2" command
...
## Updating the active config and database from Vardot 8.1.1 to Vardot 8.1.2
```
  Given that we are in the docroot of the current Vardoc project
   ...
   Then we will have the site update with the new Vardoc 8.1.2 version
...
## First way is by updating files with Copy and Paste for Vardot 8.1.0-rc7 over Vardoc 8.1.0
```
  Given that we are at the drupal root folder for Vardoc
   When we delete all drupal core folders and files
    And we copy all new Vardoc files drupal root folder for Vardoc
...
## Updating the active config and database from Vardot 8.1.0-rc7 to Vardot 8.1.0
```
  Given that we are in the docroot of the current Vardoc project
    And all files are updated using (copy and past) or (composer require/update
   When we run drush "updb" or go to "/update.php"
...
# Updating Vardot 8.1.0-rc6 to Vardot 8.1.0-rc7
  Same steps as Updating Vardot 8.1.0-rc5 to Vardot 8.1.0-rc6 
  As this release is about
