...
So that they will be disabled and not be able to use the site.
  ...
  @javascript @local @development @staging @production
  Scenario: Check if the site admin can Administer users and activate a blocked user account Normal user to be able to accessing the site
    Given I am a logged in user with the "Site admin" user
     When I go to "/admin/people"
      And I wait
     Then I should see "People"
  ...
  @javascript @local @development @staging @production
  Scenario: Check if the Normal user user is not blocked and can login after activation.
    Given I am on "/user/login"
      And I wait
     When I fill in "Normal user" for "Username"
      And I fill in "dD.123123ddd" for "Password"
