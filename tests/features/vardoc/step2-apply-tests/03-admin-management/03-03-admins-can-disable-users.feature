...
So that they will be disabled and not be able to use the site.
  ...
  @javascript @local @development @staging @production
  Scenario: Check if the site admin can Administer users and activate a blocked user account test_authenticated to be able to accessing the site
    Given I am a logged in user with the "test_site_admin" user
     When I go to "/admin/people"
      And I wait
     Then I should see "People"
  ...
  @javascript @local @development @staging @production
  Scenario: Check if the test_authenticated user is not blocked and can login after activation.
    Given I am on "/user/login"
      And I wait
     When I fill in "test_authenticated" for "Username"
      And I fill in "dD.123123ddd" for "Password"
