...
namespace Drupal\vardoc\Config;
...
class ConfigBit implements EventSubscriberInterface, ContainerInjectionInterface {
  ...
  protected function processConfigBits(array $supportedConfigType, Config $saved_config) {
    ...
    $config_template_file = DRUPAL_ROOT . '/' . \Drupal::service('extension.list.profile')->getPath('vardoc') . '/configbit/vardoc_config_templates/' . $supportedConfigType['config_template_file'];
        ...
        foreach ($config_template_file_data['config_bits'] as $target_config_bit_name => $target_config_bit_actions) {
          ...
          if (isset($target_config_bit_actions)
            ...
            if ($save_actions > 0) {
              ...
              // Flushes plugins caches on request.
  ...
  /**
   * Validate dependencies for vardoc config template bit data.
   *
   ...
   *   Config bit file source eg. vardoc.
   *
   * @return array|int
   *   array of config bit.
   *   FALSE if we do not have the config_bit in the config bit file.
   */
  public static function getConfigBit($config_bit_file_name, $type = 'profile', $project = 'vardoc') {
  ...
  /**
   * Do we have this config bit file.
   *
   * @param string $config_bit_file_name
   *   Config bit file name in the root configbit folder.
   * @param string $type
   ...
   */
  public static function doWeHaveThisConfigBit($config_bit_file_name, $type = 'profile', $project = 'vardoc') {
    // Generate full path to config file.
    $full_config_bit_file_name = \Drupal::service('extension.list.' . $type)->getPath($project) . '/' . $config_bit_file_name;
   ...
   */
  public static function getList($config_bit_file_name, $condition_name, $condition_value, $sublist = NULL, $type = 'profile', $project = 'vardoc') {
   ...
   *   The project.
   */
  public static function actionArchiveFiles($config_bit_file_name, $condition_name, $condition_value, $type = 'profile', $project = 'vardoc') {
    $config_bit_data = ConfigBit::getConfigBit($config_bit_file_name, $type, $project);
   ...
   *   The project.
   */
  public static function actionUnArchiveFiles($config_bit_file_name, $condition_name, $condition_value, $type = 'profile', $project = 'vardoc') {
    $config_bit_data = ConfigBit::getConfigBit($config_bit_file_name, $type, $project);
    if (isset($config_bit_data['type'])
        && $config_bit_data['type'] = 'action'
        && isset($config_bit_data['action'])
   ...
   *   The project.
   */
  public static function actionAdd($config_bit_file_name, $condition_name, $condition_value, $target, $type = 'profile', $project = 'vardoc') {
    $config_bit_data = ConfigBit::getConfigBit($config_bit_file_name, $type, $project);
   ...
   *   The project.
   */
  public static function actionRemove($config_bit_file_name, $condition_name, $condition_value, $target, $type = 'profile', $project = 'vardoc') {
    $config_bit_data = ConfigBit::getConfigBit($config_bit_file_name, $type, $project);
